...
pacman::p_load(bslib, bsicons, ggplot2, plotly, readxl, shiny)
```

El siguiente comando permite ver una aplicación de prueba y configurar los colores, las fuentes y todo lo estético. Se pone comentado porque ya no se ocupa.
...
server <- function(input, output, session) {
  observe({
    if (!is.null(input$edad) && !is.na(input$edad) && is.numeric(input$edad)) {
      if (input$edad < 15) {
  ...
  observe({
    if (!is.null(input$monto) && !is.na(input$monto) && is.numeric(input$monto)) {
      if (input$monto < 0) {
