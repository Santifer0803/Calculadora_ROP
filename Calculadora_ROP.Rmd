...
#'         capital_disponible: capital restante al final de cada año.
renta.tmprl <- function(capital.disp, sexo, edad) {
  ...
  # Se obtienen los años esperados residuales de la persona (datos que están en la fila 118 de los dataframes)
  esperanza.restante <- round(as.numeric(tpx[118, edad]))

  # estos son los vectores que almacenarán la información
  ...
  for (i in 1:(esperanza.restante + 1)) {
    ...
    # si el capital se agota, la pensión es cero
    if (capital.disp <= 0) {
    ...
    # VANU para la edad actual
    n <- esperanza.restante - i 
    vanu <- calculo.vanu(edad.actual, n, 12, tpx)
    
    pension.por.renta.temp <- capital.disp / vanu  

    # se compara con al pensión mínima
  pension <- ifelse(pension.por.renta.temp < 30638.35, 30638.35, pension.por.renta.temp)
    
    pension.anual <- pension * 12
    pension.anual <- min(pension.anual, capital.disp)  # para evitar pensiones > capital
    
    # se guardan los datos
    vector.pensiones[i] <- pension
    capital.disp <- capital.disp - pension.anual
    vector.capitales[i] <- capital.disp
  }

  # si llegara a pasar q la persona sobrevive más de la esperanza de vida:
  ...
  return(data.frame(
    ...
    capital_disponible = vector.capitales
  ))
}
...
```{r ejemplo_c}
c <- renta.tmprl(123456789.69420,"Mujer",65)
