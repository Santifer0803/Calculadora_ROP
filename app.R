...
tpx.h <-
  read_excel("data/VANUs 2025.xlsx", sheet = "Sobrevivencia H")
tpx.m <-
  read_excel("data/VANUs 2025.xlsx", sheet = "Sobrevivencia M")
...
calculo.vanu <- function(x, n, m, tpx) {
  ...
  for (edad in x) {
    ...
    vanu <-
      c(vanu, 12 * (sum(tpx[1:n, edad + 1] / (1 + r) ^ (0:(
        n - 1
      ))) -
        ((m - 1) / (2 * m) * (1 - (
          tpx[n + 1, edad + 1] / ((1 + r) ^ n)
        )))))
...
renta.prmnt <- function(monto.acum, edad, rends.3a) {
  ...
  data.frame(edad = edad:115,
             pension = vector.pensiones[1:(116 - edad)],
...
renta.tmprl <- function(capital.disp, sexo, edad) {
  ...
  for (i in 1:(esperanza.restante + 1)) {
    ...
    pension <-
      ifelse(pension.por.renta.temp < monto.min,
             monto.min,
             pension.por.renta.temp)
  ...
  if (capital.restante > 0) {
    ...
    vector.rendimientos <-
      c(vector.rendimientos,
        (capital.restante) * r - 12 * (capital.restante / 12) * r / 2)
  }
  ...
  vector.rendimientos <-
    vector.rendimientos[-length(vector.rendimientos)]
...
server <- function(input, output, session) {
  ...
  output$grafico_pension <- renderPlotly({
    ...
    fig <- ggplot(df, aes(x = edad, y = pension)) +
      ...
      labs(title = "Evolución de la Pensión Mensual", x = "Edad (Años)", y = "Pensión (Colones)") +
      theme_minimal()
  ...
  output$grafico_reserva <- renderPlotly({
    ...
    fig <- ggplot(df, aes(x = edad, y = reserva())) +
      geom_line(color = "#18A777", linewidth = 1.2) +
      labs(title = "Evolución de la Reserva", x = "Edad (Años)", y = "Reserva final (Millones de colones)") +
      theme_minimal()
